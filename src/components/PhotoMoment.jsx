...
export default function PhotoMoment({
  ...
  const url = useMemo(() => publicImage(photoSrc), [photoSrc])
  const label = useMemo(() => photoLabelFromSrc(photoSrc), [photoSrc])

  const aspect =
    layout === 'landscape' ? 'aspect-[16/10]' : layout === 'square' ? 'aspect-square' : 'aspect-[4/5]'
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <div className={`relative ${aspect} w-full overflow-hidden rounded-2xl bg-white/35`}>
          {!failed ? (
            <img
              src={url}
